...
dat <- dat %>% select(all_of(unique(c(ID_vars, cluster_var, covars, covars_for_smoke, 
                                      ...
                                      independent_vars_smoke, outcome_vars, table1_explan, table1_dep, "discharge_bundle"))))
...
dat <- dat %>% mutate(# patient_ID = ff_label(, ""),                      
                      ...
                      RSR_BPT_bin = ff_label(RSR_BPT_bin, "Patient reviewed by a respiratory specialist within 24 hours of arrival"),
...
dat %>% select(RSR_bin, discharge_bundle_yes_no) %>% table() %>% prop.table(1)
# 
...
# look at the FU elements

dat$FU_combos <- paste0("2 day FU: ", dat$DB_comm_FU_2_days, ", 4 week FU: ", dat$DB_spec_review_4_weeks)

table(dat$FU_combos)



FU_table_combos <-  dat %>% mutate(across(all_of(c(independent_vars[c(-1, -5)], independent_vars_smoke)),
                                          ~fct_recode(factor(., levels = c("1", "0")), `No` = "0", `Yes` = "1"))) %>%
  mutate(discharge_bundle_yes_no = factor(discharge_bundle_yes_no, levels = c("Yes", "No"))) %>%
  ff_relabel_df(dat) %>% # used to relabel after tidyverse functions remove the labels
  summary_factorlist(., dependent = "FU_combos",
                     explanatory = c(table1_explan, table1_dep, "discharge_bundle"), cont_nonpara = TRUE, total_col = TRUE,
                     add_col_totals = TRUE, p = TRUE)  

colnames(FU_table_combos)[3:6] <- c("No follow-up", "4 week specialist review only", "2 day community care referral only", "2-day and 4-week follow-up")

# write.csv(FU_table_combos, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/Tidy results/Follow-up_focus_table.csv",
#           row.names = FALSE)
...
# Checking the veracity of the results is hashed out below
###############
# adjusted analyses - section hashed out now as was checking the confidence intervals 
# can confirm after checking multiple optimisers, profile confidence intervals, and bootstrap confidence intervals,
# that the convergence warnings are false positives and that the Wald confidence intervals are fine
# 
# 
#   
#   
...
# 
# analyses_df_adj_profile <- analyses_df_adj
# analyses_df_adj_Wald <- analyses_df_adj
...
# 
# 
# # glm(formula(paste(analyses_df_unadj$outcome_vars[i]," ~ ",
# #                   analyses_df_unadj$independent_vars[i]," + ",
# #                   paste(covars, collapse = "+"))),
# #     family = binomial(link = "logit"), data = dat) %>% plot()
# 
# 
# # # let's look at the residual plots one by one and assess whcih variable is the issue
# # 
# # glm(formula(paste(analyses_df_unadj$outcome_vars[i]," ~ ",
# #                   analyses_df_unadj$independent_vars[i]," + ",
# #                   paste(covars[c(1,3)], collapse = "+"))),
# #     family = binomial(link = "logit"), data = dat) %>% plot()
# # 
# # glm(formula(paste(analyses_df_unadj$outcome_vars[i]," ~ ",
# #                   analyses_df_unadj$independent_vars[i]," + ",
# #                   paste(covars[c(1,3)], collapse = "+"))),
# #     family = binomial(link = "probit"), data = dat) %>% plot()
# 
# 
# 
# # for (i in 1:3) {
...
# # 
# # 
# #   try({  print(tidyoutput(x = model_output_list_adj[[i]], meth = "Wald", MEM = TRUE, pval = TRUE, roundno = 2))  })
# #   try({    analyses_df_adj_Wald[i, 3:7] <- tidyoutput(x = model_output_list_adj[[i]], meth = "Wald", MEM = TRUE, pval = TRUE, roundno = 2)[2, ]  })
# #   try({  analyses_df_adj_profile[i, 3:7] <- tidyoutput(x = model_output_list_adj[[i]], meth = "profile", MEM = TRUE, pval = TRUE, roundno = 2)[2, ]  })
# #   try({  analyses_df_adj_glm[i, 3:7] <- tidyoutput(x = model_output_list_adj_glm[[i]], meth = "profile", MEM = FALSE, pval = TRUE, roundno = 2)[2, ]  })
# # 
# # }
# # 
# # saveRDS(analyses_df_adj_Wald, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj_Wald_1-20.RDS")
# # saveRDS(analyses_df_adj_profile, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj_profile_1-20.RDS")
# # saveRDS(analyses_df_adj_glm, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj_glm_1-20.RDS")
# # saveRDS(model_output_list_adj, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/model_output_list_adj_1-20.RDS")
# # saveRDS(model_output_list_adj_glm, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/model_output_list_adj_glm_1-20.RDS")
# 
# 
# 
# analyses_df_adj_Wald <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj_Wald_1-20.RDS")
# analyses_df_adj_profile <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj_profile_1-20.RDS")
# analyses_df_adj_glm <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj_glm_1-20.RDS")
# model_output_list_adj <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/model_output_list_adj_1-20.RDS")
# model_output_list_adj_glm <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/model_output_list_adj_glm_1-20.RDS")
# 
# 
# 
# analyses_df_adj_Wald <- analyses_df_adj_Wald %>% clean_up() %>% select(outcome_vars, independent_vars, coefficients, pval)
# analyses_df_adj_profile <- analyses_df_adj_profile %>% clean_up() %>% select(outcome_vars, independent_vars, coefficients, pval)
# analyses_df_adj_glm <- analyses_df_adj_glm %>% clean_up() %>% select(outcome_vars, independent_vars, coefficients, pval)
# 
# analyses_df_adj_compared <- analyses_df_adj_Wald[ ,-4] %>% rename(Wald_coefficients = coefficients) %>%
#   left_join(., rename(analyses_df_adj_profile[ ,-4], profile_coefficients = coefficients), 
#             by = c("outcome_vars", "independent_vars")) %>%
#   left_join(., rename(analyses_df_adj_glm[ ,-4], glm_coefficients = coefficients), 
#             by = c("outcome_vars", "independent_vars"))
# 
# 
# 
# 
# model_output_list_adj[[1]]
# 
# dat %>% group_by(hosp_code) %>% summarise(n=n()) %>% pull(n) %>% table()
# 
# # check other optimizers
# 
# 
# 
# conv_check <- allFit(model_output_list_adj[[1]], verbose = TRUE) # all the results are the same
# 
# 
# 
# check_resid <- ranef(model_output_list_adj[[1]])
# dotplot.ranef.mer(check_resid)
# plot(model_output_list_adj[[1]])
# 
# plot(check_resid)
# 
# str(check_resid)
# library(lme4)
# qqmath(ranef(model_output_list_adj[[1]]))
# qqline(ranef(model_output_list_adj[[1]]))
# 
# # even though the random effects lie off the line towards the tails, due to the
# # large sample size central limit theorum says that this probably isn't an issue 
# 
# mod <- model_output_list_adj[[1]]
# 
# 
# cl <- makeCluster(14)
# 
...
#                 nsim = 500,
#                 FUN = fixef,
#                 parallel = "snow", cl = cl, verbose = TRUE,
#                 ncpus = 14,
#                 use.u = TRUE)
#                 
# # saveRDS(boot, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/bootstrapcheck.RDS")
#             
# boot    
# 
# summary(boot)
...
# library(boot)
# 
# boot.ci(boot, type = c("perc", "norm", "basic"))
# 
# fixed <- fixef(mod)
# ci_list <- list()
# 
# for (i in 1:length(fixed)) {
#   ci <- boot.ci(boot, index = i, type = c("perc", "norm", "basic"))
#   ci_list[[names(fixed)[i]]] <- ci
# }
# 
# ci_list$`inhaled_steroids_disNot prescribed for medical reasons`[3]
# 
# param_names <- names(fixed)
# point_estimates <- fixed
# ci_lower <- sapply(ci_list, function(x) x$percent[4])
# ci_upper <- sapply(ci_list, function(x) x$percent[5])
# 
# ci_df <- data.frame(
...
#   hi = ci_upper)
# 
# ci_df <- ci_df %>% mutate(est = exp(est), lo = exp(lo), hi = exp(hi))
# ci_df$pval <- 1 # not relevant but needed for the function
# ci_df <- clean_up(ci_df)
# ci_df$pval <- NULL
# ci_df <- ci_df %>% rename(bootstrap = coefficients)
# # moment of truth - how do they compare?
# 
# mod_wald <- tidyoutput(x = mod, meth = "Wald", MEM = TRUE, pval = TRUE, roundno = 2)
# mod_wald <- clean_up(mod_wald)
# 
# boot_wald_comp <- left_join(mod_wald, ci_df, by = c("Variable" = "parameter"))
# 
# # thank god!!! very comparable!!! 
# 
# boot_wald_comp
# (ci_list$BPT2024$percent)
# (ci_list$BPT2024$norm)
# (ci_list$BPT2024$basic)
# 
# 
# plot(ranef(model_output_list_adj[[1]]))
# 
# ranef(model_output_list_adj[[1]], condVar = TRUE)
# ranef(model_output_list_adj[[1]], condVar = FALSE)
# 
# summary(ranef(model_output_list_adj[[1]], condVar = TRUE))
##############
...
saveRDS(analyses_df_adj, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj.RDS")
saveRDS(analyses_df_adj_glm, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj_glm.RDS")
saveRDS(model_output_list_adj, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/model_output_list_adj.RDS")
saveRDS(model_output_list_adj_glm, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/model_output_list_adj_glm.RDS")




analyses_df_adj <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj.RDS")
analyses_df_adj_glm <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/analyses_df_adj_glm.RDS")
...
model_output_list_adj_glm <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/model_output_list_adj_glm.RDS")
...
saveRDS(analyses_df_adj_smoking, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/analyses_df_adj_smoking.RDS")
saveRDS(analyses_df_adj_smoking_glm, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/analyses_df_adj_smoking_glm.RDS")
saveRDS(model_output_list_adj_smoking, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/model_output_list_adj_smoking.RDS")
saveRDS(model_output_list_adj_smoking_glm, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/model_output_list_adj_smoking_glm.RDS")


analyses_df_adj_smoking <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/analyses_df_adj_smoking.RDS")
analyses_df_adj_smoking_glm <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/analyses_df_adj_smoking_glm.RDS")
model_output_list_adj_smoking <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/model_output_list_adj_smoking.RDS")
model_output_list_adj_smoking_glm <- readRDS("G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/model_output_list_adj_smoking_glm.RDS")
...
write.csv(clean_adj_analyses, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/Tidy results/Clean_adjusted_results_table.csv",
          row.names = FALSE)
...
"RSR_BPT_bin" =  "Respiratory specialist review within 24 hours of arrival",
...
covars_for_all_elements_included <- covars_for_all_elements_included[c(-5, -6)]
...
# use to extract R2 for model that excludes DB elements


extract.R2(model_all_DB_indiv[[1]]) # read30
extract.R2(model_all_DB_indiv[[2]]) # asthma read30
extract.R2(model_all_DB_indiv[[3]]) # read90
extract.R2(model_all_DB_indiv[[4]]) # asthma read90
