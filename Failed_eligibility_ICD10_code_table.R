...
ICD10_vars <- c("asthma_any_diag_position", "other_ICD10s")

ICD10_table1_explan <- c("IMD_quintile", "age_orig", "gender", "smoke_status", "inhaled_steroids_dis",                        
                 "oral_steroids_dis", "oral_steroids_rescue_history",
                 "asthma_sev", "CCIweightedcat", # (this is covars but with original age) 
                 independent_vars, independent_vars_smoke, outcome_vars_table1, ICD10_vars)


dat$ICD10_eligibile_for_audit <- "Yes"
dat$ICD10_eligibile_for_audit[dat$adult_asthma_coded_admission_including_index == 0] <- "No"
dat$ICD10_eligibile_for_audit <- factor(dat$ICD10_eligibile_for_audit, levels = c("No", "Yes"))


ICD10_table_dep <- "ICD10_eligibile_for_audit"
...
nrow(dat)
dat %>% filter(ICD10_eligibile_for_audit == "No") %>% group_by(DIAG_01) %>% summarise(n = n()) %>%
  arrange(desc(n)) %>% print(n=1000) 


# create the levels for the top 10 non-asthmatic ICD10 codes so they are displayed in order
ICD10_levels <- dat %>% filter(adult_asthma_coded_admission_including_index == 0) %>% mutate(levelsss = fct_lump(DIAG_01, n=10)) %>%
  group_by(levelsss) %>% summarise(n = n()) %>%
  arrange(desc(n)) %>% mutate(levelsss = as.character(levelsss)) %>% pull(levelsss)

ICD10_levels <- ICD10_levels[c(2:11, 1)]
  

dat <- dat %>% mutate(other_ICD10s = if_else(adult_asthma_coded_admission_including_index == 0,
                                            DIAG_01, NA)) %>%
  mutate(other_ICD10s = fct_lump(other_ICD10s, n=10)) %>%
  mutate(other_ICD10s = factor(other_ICD10s,  levels = ICD10_levels))

table(dat$other_ICD10s)

dat %>% filter(other_ICD10s == "Other") %>% select(DIAG_01_3d) %>% group_by(DIAG_01_3d) %>%
  summarise(n = n()) %>% arrange(desc(n)) %>% pull() %>%  sum()

dat <- dat %>% mutate(other_ICD10s_check = if_else(adult_asthma_coded_admission_including_index == 1,
                                             DIAG_01, NA)) %>%
  mutate(other_ICD10s_check = fct_lump(other_ICD10s_check, n=10)) 


table(dat$other_ICD10s_check, useNA = "ifany")

# J450  J459  J46X

dat <- dat %>% rowwise() %>% 
  mutate(asthma_any_diag_position = as.integer(any(c("J450", "J459", "J46X") %in% c_across(DIAG_01:DIAG_20)))) %>%
  ungroup()


table(dat$asthma_any_diag_position)

dat %>% filter(adult_asthma_coded_admission_including_index == 0) %>%
  group_by(asthma_any_diag_position) %>% slice(1:30) %>% select(DIAG_01:DIAG_20) %>% print(n=1000)


table(dat$asthma_any_diag_position)

dat %>% filter(adult_asthma_coded_admission_including_index == 0) %>%
  select(discharge_bundle_yes_no, asthma_any_diag_position) %>% table()


# dat <- dat %>% select(all_of(unique(c(ID_vars, cluster_var, covars, covars_smoke, 
...
dat <- dat %>% mutate(# patient_ID = ff_label(, ""),                      
                      ...
                      asthma_any_diag_position = ff_label(asthma_any_diag_position, "Asthma code present in any diagnosis position"),
                      other_ICD10s = ff_label(other_ICD10s, "Most-used ICD10 codes in the first diagnosis position in those without a primary diagnosis of asthma"),
                      ICD10_eligibile_for_audit = ff_label(ICD10_eligibile_for_audit, "Patient has asthma code as their primary diagnosis"))
...
ICD10_table <-  dat %>% mutate(across(all_of(c(independent_vars[c(-1, -5)], independent_vars_smoke, "asthma_any_diag_position")),
  ...
  summary_factorlist(., dependent = ICD10_table_dep,
                       explanatory = ICD10_table1_explan, cont_nonpara = TRUE, total_col = FALSE,
                       add_col_totals = TRUE, p = FALSE, fit_id = TRUE)  

# then we remove the ICD10 primary codes variable and fit the p values (as it will crash otherwise)
ICD10_table_ps <-  dat %>% mutate(across(all_of(c(independent_vars[c(-1, -5)], independent_vars_smoke, "asthma_any_diag_position")),
                                      ~fct_recode(factor(., levels = c("1", "0")), `No` = "0", `Yes` = "1"))) %>%
  mutate(discharge_bundle_yes_no = factor(discharge_bundle_yes_no, levels = c("Yes", "No"))) %>%
  ff_relabel_df(dat) %>% # used to relabel after tidyverse functions remove the labels
  summary_factorlist(., dependent = ICD10_table_dep,
                     explanatory = ICD10_table1_explan[-30], cont_nonpara = TRUE, total_col = FALSE,
                     add_col_totals = TRUE, p = TRUE, fit_id = TRUE)  

ICD10_table_ps <- ICD10_table_ps %>% select(p, fit_id, index)

ICD10_table <- left_join(ICD10_table, ICD10_table_ps, by = c("fit_id", "index"))
glimpse(ICD10_table)
glimpse(ICD10_table_ps)

ICD10_table <- ICD10_table %>% select(-fit_id, -index)


write.csv(ICD10_table, "G:/Alex Harley/Audit_2023_onwards/2022-2023/Research/Adult_asthma_BPT/Analysis/Output/Analysis for paper/Tidy results/Supplement_Table_asthma_vs_non_asthma_coded.csv",
          row.names = FALSE)
